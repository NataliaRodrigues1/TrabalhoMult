---
title: ''
geometry: left=18mm, right=19mm, top=42mm, bottom=40mm
header-includes:
- \usepackage{setspace}
- \usepackage{pslatex}
...
\newpage
  ...
  Na figura 2, estão disponibilizados os boxplots referentes às variáveis de interesse para ambos os grupos. A princípio, nota-se a grande presença de outliers para a espécie Carteri em seis dos 7 gráficos, enquanto que a espécie Torrens apresenta outlies em apenas um deles. Além disso, também para seis das sete variáveis, a espécie Carteri apresenta valores superiores da mediana em relação à espécie Torrens, nao ocorrendo tal evento apenas para a variável  C13a.
  Ainda utilizando os boxplots (figura 2), pode-se identificar, pelas distâncias entre os primeiros e terceiros quartis em relação à mediana, que há bastante assimetria nas distribuições, e que a distribuições das variáveis aparentam ser diferentes entre as espécies, sendo que as manores diferenças parecem estar nas caracterísicas C3p e C4p.
...
A figura 3 mostra os gráficos de quantis-quantis com envelopes para a distância de Mahalanobis, para cada espécie. É perceptível que a suposição de normalidade multivariada não aparenta ser razoável para cada uma das espécies. Nota-se, nestes gráficos, que há diversas fugas para quantis menores na parte inferior do gráfico, e de quantis maiores na parte superior do gráfico da distribuição F, o que nos indica que a suposição de normalidade multivariada pode nao ser adequada aos dados.
...
Diante destas análises podemos conjecturar que exita, entre as espécies, diferença na média de variáveis como C.Asa, C3p e C4p. Para verificar a validade ou não tais conjecturas faremos uma análise inferencial dos dados.
...
Para realizar a supracitada análise, foram utilizados testes do tipo CBU=M (ver Azevedo(2017)) considerando um nível de significância $\alpha=0,05$ cujos resultados são mostrados na tabela (numero da tabela), e por meio destes conclui-se que quatro das sete variáveis possuem as médias estatisticamente iguais em relação as duas espécies, sendo estas as mesmas indicadas pelos testes de significancia dos parâmetros, ou seja, L.Asa, L3p, C12a e C13a. Para as demais (C.Asa, C3p, C4p), tem-se evidência que existe diferença entre os grupos estudados.
...
Na figura 4 estão os gráficos de médias preditas pelo modelo completo para cada variável, por grupo. Os intervalos de confiança assintóticos apresentados foram calculados utilizando a metodologia estudada em sala e descrita em (Azevedo(2017)).
  Para as variáveis C3p e C4p pode-se notr que a espécie Carteri tem uma maior média predita e que seus intervalos de confiança nao se interseptam indicando que a distribuição destas variáveis é consideravelmente diferente entre os grupos.
  Apesar de o teste de igualdade de médias ter rejeitado, a um nivel de 5% de confiança, a hipótese de igualdade para a variável C.Asa, no gráfico essa diferença nao é tçao explicita e os intervalos de confiança para os dois grupos chegam se interseptar.
  Para as outras variáveis as medias preditas são muito próximas e os IC's tem grandes intersecções o que está de acordo com os resultados dos testes de igualdade de medias feitos acima.
  
COLOCAR GRÁFICO DE MEDIAS PREDITAS!
  
  As figuras de 5 a 11 apresentam os gráficos de resíduo do modelo para cada variável, onde temo, da esqueda para a direita, de cima para baixo, os seguintes gráficos (A)Resíduos Studentizado x Índice, (B)Resíduos Studentizado x Valor Ajustado, (C)Hístograma dos Resíduos Studentizado e (D)Q-Qplot da distríbuição normal. A partir de agora usaremos as respectivas letras (A, B, C e D) para nos referirmos a cada um dos gráficos.
  Análisando os gráficos A e B nota-se que para algumas variáveis o comportamento destes gráficos indicam a presença de heterocedasticidade e dependencia dos resíduos, como por exemplo para C4p para a qual o gráfico A aprensenta um aumento da dispersão dos pontos com o aumento dos indíces, ficando com um formato de "cone" e no gráfico B a variabilidade dos resíduos para um dos grupos é bem maior que para o outro. Nas variáveis C.Asa e c3p também é observado um aumento da variábilidade entre os grupos no gráfico B e, mais especificamente para a variável C3p uma tendência em A parecida com a que foi observada em C4p.
  ...
  No grafico D observa-se padrões no comportamento dos resíduos de todas as variáveis, e para as variáveis C.Asa, L3p, C4p e C12a também é possível notar diversos pontos fora das bandas de confiança. Estes resultados evidênciam a não validade da suposição de normalidade dos resíduos.
...
```{r, fig.height=5, fig.cap="Gráfico de Resíduos para a variável C.Asa", echo=FALSE}
# Comprimento da Asa
par(mfrow =c(2,2))
...
```{r, fig.cap="Gráfico de Resíduos para a variável L.Asa", echo=FALSE}
# Largura da Asa
par(mfrow =c(2,2))
...
```{r, fig.cap="Gráfico de Resíduos para a variável C3p", echo=FALSE}
# Comprimento do 3 Palpo
par(mfrow =c(2,2))
...
```{r, fig.cap="Gráfico de Resíduos para a variável L3p", echo=FALSE}
# Largura do 3 Palpo
par(mfrow =c(2,2))
...
```{r, fig.cap="Gráfico de Resíduos para a variável C4p",  echo=FALSE}
# Comprimento do 2 Palpo
par(mfrow =c(2,2))
...
```{r, fig.cap="Gráfico de Resíduos para a variável C12a", echo=FALSE}
...
```{r, fig.cap="Gráfico de Resíduos para a variável C13a", echo=FALSE}
# Comprimento do 13 segmento da antena
par(mfrow =c(2,2))
